...
#include "cores/RetroPlayer/VideoRenderers/RPBaseRenderer.h"
...
public:
  ...
  virtual void UpdateRenderInfo(CRPRenderInfo& info) = 0;
...
{
public:
  CRPRenderManager(CDVDClock &clock, IRetroPlayerRenderMsg *player);
  ~CRPRenderManager();
  ...
  /**
   * Called by RetroPlayer to configure renderer
   * @param picture
   * @param fps frames per second of video
   * @param flags see RPRenderFlags.h
   * @param orientation
   * @param buffers number of kept buffer references
   */
  bool Configure(RPVideoPicture& picture, float fps, unsigned flags, unsigned int orientation, int buffers = 0);

  int AddRPVideoPicture(RPVideoPicture& picture);

  /**
   * Called by RetroPlayer to flip render buffers
   ...
   * @param timestamp of frame delivered with AddRPVideoPicture
   * @param pts used for lateness detection
   * @param deintMethod method for deinterlacing
   * @param sync signals frame, top, or bottom field
   * @param wait: block until pic has been rendered
   */
  void FlipPage(volatile std::atomic_bool& bStop, double pts, EINTERLACEMETHOD deintMethod, ERPFIELDSYNC sync, bool wait);

  // Get renderer info, can be called before configure
  CRPRenderInfo GetRenderInfo();
  ...
  /**
   * RetroPlayer call this on flush in oder to discard any queued frames
   */
  void DiscardBuffer();

  void SetDelay(int delay) { m_videoDelay = delay; };
  int GetDelay() const { return m_videoDelay; };
...
protected:
  ...
  IRetroPlayerRenderMsg *m_playerPort;

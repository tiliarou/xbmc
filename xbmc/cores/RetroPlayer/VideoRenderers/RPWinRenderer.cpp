...
CRPWinRenderer::CRPWinRenderer()
{
  ...
  m_iRequestedMethod = RP_RENDER_METHOD_AUTO;
...
static enum AVPixelFormat PixelFormatFromFormat(ERPRenderFormat format)
{
  if (format == RP_RENDER_FMT_DXVA)      return AV_PIX_FMT_NV12;
  if (format == RP_RENDER_FMT_YUV420P)   return AV_PIX_FMT_YUV420P;
  if (format == RP_RENDER_FMT_YUV420P10) return AV_PIX_FMT_YUV420P10;
  if (format == RP_RENDER_FMT_YUV420P16) return AV_PIX_FMT_YUV420P16;
  if (format == RP_RENDER_FMT_NV12)      return AV_PIX_FMT_NV12;
  if (format == RP_RENDER_FMT_UYVY422)   return AV_PIX_FMT_UYVY422;
  if (format == RP_RENDER_FMT_YUYV422)   return AV_PIX_FMT_YUYV422;
  return AV_PIX_FMT_NONE;
}

bool CRPWinRenderer::HandlesRenderFormat(ERPRenderFormat format)
{
  if ( format == RP_RENDER_FMT_DXVA
    || format == RP_RENDER_FMT_YUV420P
    || format == RP_RENDER_FMT_YUV420P10
    || format == RP_RENDER_FMT_YUV420P16
    || format == RP_RENDER_FMT_NV12
    || format == RP_RENDER_FMT_UYVY422
    || format == RP_RENDER_FMT_YUYV422)
  {
    return true;
  }
  return false;
...
bool CRPWinRenderer::ConfigChanged(void *hwPic)
{
  // Disable DXVA rendererer
  /*
...
void CRPWinRenderer::ManageTextures()
{
...
void CRPWinRenderer::SelectRenderMethod()
{
  ...
  else */
  {
    ...
    */
    case RP_RENDER_METHOD_AUTO:
    case RP_RENDER_METHOD_D3D_PS:
    {
      CRPTestShader shader;
      if (shader.Create())
      {
        m_renderMethod = RENDER_PS;
        if (m_format == RP_RENDER_FMT_DXVA)
          m_format = RP_RENDER_FMT_NV12;
        break;
      }
      else
      {
    ...
    case RP_RENDER_METHOD_SOFTWARE:
    default:
...
bool CRPWinRenderer::Configure(unsigned int width, unsigned int height, unsigned int d_width, unsigned int d_height, float fps, unsigned flags, ERPRenderFormat format, void *hwPic, unsigned int orientation)
{
  m_sourceWidth = width;
...
int CRPWinRenderer::NextYV12Texture()
{
  if (m_NumYV12Buffers)
...
bool CRPWinRenderer::IsPictureHW(RPVideoPicture &picture)
{
  // Disable DXVA rendererer
  /*
  if (m_renderMethod == RENDER_DXVA
    || picture.format == RP_RENDER_FMT_DXVA)
  {
    return true;
  }
  */
...
void CRPWinRenderer::AddRPVideoPictureHW(RPVideoPicture &picture, int index)
{
  // Disable DXVA rendererer
  /*
  if (m_renderMethod == RENDER_DXVA)
...
int CRPWinRenderer::GetImage(YV12ImageRP *image, int source, bool readonly)
{
  ...
  */
  ...
  image->flags = 0;
  if (m_format == RP_RENDER_FMT_YUV420P10
    || m_format == RP_RENDER_FMT_YUV420P16)
    image->bpp = 2;
  else
    image->bpp = 1;
...
void CRPWinRenderer::ReleaseImage(int source, bool preserve)
{
}

void CRPWinRenderer::Reset()
{
}

void CRPWinRenderer::Update()
{
  if (!m_bConfigured)
    return;
  ManageRenderArea();
...
void CRPWinRenderer::RenderUpdate(bool clear, unsigned int flags, unsigned int alpha)
{
  if (clear)
...
void CRPWinRenderer::FlipPage(int source)
{
...
void CRPWinRenderer::PreInit()
{
  ...
  if (g_advancedSettings.m_DXVAForceProcessorRenderer
    || m_iRequestedMethod == RP_RENDER_METHOD_DXVA)
  {
    m_processor = new DXVA::CRPProcessorHD();
    if (!m_processor->PreInit())
    {
      CLog::Log(LOGNOTICE, "CRPWinRenderer::Preinit - could not init DXVA processor - skipping");
  ...
  */
  ...
  // If DXVA was the requested renderer, default to the pixel renderer to still make use of h/w acceleration
  if (m_iRequestedMethod == RP_RENDER_METHOD_DXVA)
    m_iRequestedMethod = RP_RENDER_METHOD_D3D_PS;
  ...
  {
    m_formats.push_back(RP_RENDER_FMT_YUV420P10);
...
bool CRPWinRenderer::CreateIntermediateRenderTarget(unsigned int width, unsigned int height, bool dynamic)
{
...
void CRPWinRenderer::SelectSWVideoFilter()
{
...
void CRPWinRenderer::SelectPSVideoFilter()
{
  m_bUseHQScaler = false;
...
void CRPWinRenderer::UpdatePSVideoFilter()
{
  ...
  if (m_bUseHQScaler)
  {
    ...
    m_scalerShader = new CRPConvolutionShaderSeparable();
    ...
    {
      m_scalerShader = new CRPConvolutionShader1Pass();
  ...
  */
  ...
  m_colorShader = new CRPYUV2RGBShader();
...
void CRPWinRenderer::UpdateVideoFilter()
{
...
void CRPWinRenderer::Render(DWORD flags)
{
...
void CRPWinRenderer::RenderSW()
{
  ...
  YUVBufferRP* buf = reinterpret_cast<YUVBufferRP*>(m_VideoBuffers[m_iYV12RenderBuffer]);
...
void CRPWinRenderer::RenderPS()
{
  ...
  m_colorShader->Render(m_sourceRect, destPoints,
    ...
    m_iFlags, reinterpret_cast<YUVBufferRP*>(m_VideoBuffers[m_iYV12RenderBuffer]),
    m_scalingMethod);
...
void CRPWinRenderer::RenderHW(DWORD flags)
{
...
//********************************************************************************************************
void CRPWinRenderer::DeleteYV12Texture(int index)
{
  CSingleLock lock(g_graphicsContext);
  SAFE_DELETE(m_VideoBuffers[index]);
}

bool CRPWinRenderer::CreateYV12Texture(int index)
{
  CSingleLock lock(g_graphicsContext);
  ...
  // else
  // {
  YUVBufferRP *buf = new YUVBufferRP();
...
bool CRPWinRenderer::Supports(ERENDERFEATURE feature)
{
...
bool CRPWinRenderer::Supports(ESCALINGMETHOD method)
{
  // Disable DXVA rendererer
...
bool CRPWinRenderer::WantsDoublePass()
{
  // Disable DXVA rendererer
  /*
  if (m_renderMethod == RENDER_DXVA)
...
CRPRenderInfo CRPWinRenderer::GetRenderInfo()
{
  CRPRenderInfo info;
...
void CRPWinRenderer::ReleaseBuffer(int idx)
{
  // Disable DXVA rendererer
  /*
...
bool CRPWinRenderer::NeedBuffer(int idx)
{
  // Disable DXVA rendererer
  /*
...
YUVBufferRP::~YUVBufferRP()
{
  Release();
}

bool YUVBufferRP::Create(ERPRenderFormat format, unsigned int width, unsigned int height, bool dynamic)
{
  m_format = format;
  m_width = width;
  m_height = height;
  ...
  switch (m_format)
  {
  case RP_RENDER_FMT_YUV420P10:
  case RP_RENDER_FMT_YUV420P16:
  {
  ...
  case RP_RENDER_FMT_DXVA:
  case RP_RENDER_FMT_NV12:
  {
...
bool YUVBufferRP::IsReadyToRender()
{
  return !m_locked;
}

bool YUVBufferRP::CopyFromPicture(RPVideoPicture &picture)
{
  // Disable DXVA rendererer
  /*
  if (picture.format == RP_RENDER_FMT_DXVA)
  {
...
bool YUVBufferRP::CopyFromDXVA(ID3D11VideoDecoderOutputView* pView)
{
  if (!pView)
    return false;

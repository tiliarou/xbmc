...
void CRPBaseRenderer::saveRotatedCoords()
{
  for (int i = 0; i < 4; i++)
    m_savedRotatedDestCoords[i] = m_rotatedDestCoords[i];
}

void CRPBaseRenderer::syncDestRectToRotatedPoints()
{
...
void CRPBaseRenderer::restoreRotatedCoords()
{
  for (int i = 0; i < 4; i++)
    m_rotatedDestCoords[i] = m_savedRotatedDestCoords[i];
}

void CRPBaseRenderer::CalcNormalRenderRect(float offsetX, float offsetY, float width, float height,
  ...
  if (m_format != RP_RENDER_FMT_BYPASS)
  {
    switch(stereo_mode)
    {
      case CONF_FLAGS_STEREO_MODE_TAB:
        ...
        if (m_format == RP_RENDER_FMT_CVBREF || m_format == RP_RENDER_FMT_MEDIACODEC)
        {
...
ERPShaderFormat CRPBaseRenderer::GetShaderFormat(ERPRenderFormat renderFormat)
{
  ERPShaderFormat ret = RP_SHADER_NONE;

  if (m_format == RP_RENDER_FMT_YUV420P)
    ret = RP_SHADER_YV12;
  else if (m_format == RP_RENDER_FMT_YUV420P10)
    ret = RP_SHADER_YV12_10;
  else if (m_format == RP_RENDER_FMT_YUV420P16)
    ret = RP_SHADER_YV12_16;
  else if (m_format == RP_RENDER_FMT_NV12)
    ret = RP_SHADER_NV12;
  else if (m_format == RP_RENDER_FMT_YUYV422)
    ret = RP_SHADER_YUY2;
  else if (m_format == RP_RENDER_FMT_UYVY422)
    ret = RP_SHADER_UYVY;
  else
    CLog::Log(LOGERROR, "CRPBaseRenderer::GetShaderFormat - unsupported format %d", renderFormat);

  return ret;
}

void CRPBaseRenderer::SetViewMode(int viewMode)
{

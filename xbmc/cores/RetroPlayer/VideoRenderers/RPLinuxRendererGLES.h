...
public:
  ...
  // Player functions
  virtual bool Configure(unsigned int width, unsigned int height, unsigned int d_width, unsigned int d_height, float fps, unsigned flags, ERPRenderFormat format, void* hwPic, unsigned int orientation) override;
  virtual bool IsConfigured() override { return m_bConfigured; }
  virtual int GetImage(YV12ImageRP *image, int source = AUTOSOURCE, bool readonly = false) override;
...
protected:
  ...
  typedef YUVBufferRP          YUVBufferRPS[NUM_BUFFERS];

  // YV12 decoder textures
  // field index 0 is full image, 1 is odd scanlines, 2 is even scanlines
  YUVBufferRPS m_buffers;

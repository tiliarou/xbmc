...
#include "windowing/WindowingFactory.h"
#include "RPYUV2RGBShader.h"


using namespace DirectX::PackedVector;

CRPYUV2RGBMatrix::CRPYUV2RGBMatrix()
{
  m_NeedRecalc = true;
  m_blacklevel = 0.0f;
  m_contrast = 0.0f;
...
void CRPYUV2RGBMatrix::SetParameters(float contrast, float blacklevel, unsigned int flags, ERPShaderFormat format)
{
...
XMFLOAT4X4* CRPYUV2RGBMatrix::Matrix()
{
  if (m_NeedRecalc)
  {
...
bool CRPWinShader::CreateVertexBuffer(unsigned int vertCount, unsigned int vertSize)
{
...
bool CRPWinShader::Execute(std::vector<ID3D11RenderTargetView*> *vecRT, unsigned int vertexIndexStep)
{
...
bool CRPYUV2RGBShader::Create(unsigned int sourceWidth, unsigned int sourceHeight, ERPShaderFormat fmt, ESCALINGMETHOD scalingMethod)
{
  CRPWinShader::CreateVertexBuffer(4, sizeof(CUSTOMVERTEX));
  ...
  {
  case RP_SHADER_YV12:
  case RP_SHADER_YV12_10:
  case RP_SHADER_YV12_16:
    defines["XBMC_YV12"] = "";
  ...
  if (!CRPWinShader::CreateInputLayout(layout, ARRAYSIZE(layout)))
  {
...
void CRPYUV2RGBShader::Render(CRect sourceRect, CPoint dest[],
  float contrast,
  float brightness,
  unsigned int flags,
  YUVBufferRP* YUVbuf,
...
void CRPYUV2RGBShader::HandleScalerChange(ESCALINGMETHOD scalingMethod)
{
...
CRPYUV2RGBShader::~CRPYUV2RGBShader()
{
}

void CRPYUV2RGBShader::PrepareParameters(CRect sourceRect,
  CPoint dest[],
  float contrast,
  float brightness,
  unsigned int flags)
{
    ...
    || m_dest[2] != dest[2] || m_dest[3] != dest[3])
  {
    ...
    CUSTOMVERTEX* v;
    CRPWinShader::LockVertexBuffer((void**)&v);
    ...
    CRPWinShader::UnlockVertexBuffer();
  }
...
void CRPYUV2RGBShader::SetShaderParameters(YUVBufferRP* YUVbuf)
{
...
CRPConvolutionShader::~CRPConvolutionShader()
{
  if (m_HQKernelTexture.Get())
    m_HQKernelTexture.Release();
}

bool CRPConvolutionShader::ChooseKernelD3DFormat()
{
...
bool CRPConvolutionShader1Pass::Create(ESCALINGMETHOD method)
{
  ...
  return CRPWinShader::CreateInputLayout(layout, ARRAYSIZE(layout));
}

void CRPConvolutionShader1Pass::Render(CD3DTexture &sourceTexture,
...
void CRPConvolutionShader1Pass::PrepareParameters(unsigned int sourceWidth, unsigned int sourceHeight,
  CRect sourceRect,
  CRect destRect)
{
...
void CRPConvolutionShader1Pass::SetShaderParameters(CD3DTexture &sourceTexture, float* texSteps, int texStepsCount, bool useLimitRange)
{
...
CRPConvolutionShaderSeparable::CRPConvolutionShaderSeparable() : CRPConvolutionShader()
{
  m_sourceWidth = -1;
  m_sourceHeight = -1;
  m_destWidth = -1;
...
bool CRPConvolutionShaderSeparable::Create(ESCALINGMETHOD method)
{
  std::string effectString;
  switch (method)
  {
  ...
  CRPWinShader::CreateVertexBuffer(8, sizeof(CUSTOMVERTEX));
...
void CRPConvolutionShaderSeparable::Render(CD3DTexture &sourceTexture,
  unsigned int sourceWidth, unsigned int sourceHeight,
  unsigned int destWidth, unsigned int destHeight,
  CRect sourceRect,
  CRect destRect,
...
CRPConvolutionShaderSeparable::~CRPConvolutionShaderSeparable()
{
...
bool CRPConvolutionShaderSeparable::ChooseIntermediateD3DFormat()
{
...
bool CRPConvolutionShaderSeparable::CreateIntermediateRenderTarget(unsigned int width, unsigned int height)
{
  if (m_IntermediateTarget.Get())
    m_IntermediateTarget.Release();
...
bool CRPConvolutionShaderSeparable::ClearIntermediateRenderTarget()
{
...
void CRPConvolutionShaderSeparable::PrepareParameters(unsigned int sourceWidth, unsigned int sourceHeight,
  unsigned int destWidth, unsigned int destHeight,
  CRect sourceRect,
  CRect destRect)
{
...
void CRPConvolutionShaderSeparable::SetShaderParameters(CD3DTexture &sourceTexture, float* texSteps, int texStepsCount, bool useLimitRange)
{
  m_effect.SetTechnique("SCALER_T");
  m_effect.SetTexture("g_Texture", sourceTexture);
  m_effect.SetTexture("g_KernelTexture", m_HQKernelTexture);
...
void CRPConvolutionShaderSeparable::SetStepParams(UINT iPass)
{
...
//==========================================================
#define RP_SHADER_SOURCE(...) #__VA_ARGS__

bool CRPTestShader::Create()
{
  std::string strShader = RP_SHADER_SOURCE(

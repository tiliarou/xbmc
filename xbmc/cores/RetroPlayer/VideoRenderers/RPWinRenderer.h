...
class CRPYUV2RGBShader;
class CRPConvolutionShader;
...
struct YUVBufferRP : SVideoBuffer
{
  YUVBufferRP() : m_width(0), m_height(0)
  ...
  ~YUVBufferRP();
  bool Create(ERPRenderFormat format, unsigned int width, unsigned int height, bool dynamic);
  unsigned int GetActivePlanes() { return m_activeplanes; }
  bool CopyFromPicture(RPVideoPicture &picture);
...
public:
  ...
  virtual bool IsPictureHW(RPVideoPicture &picture) override;
  ...
  virtual bool HandlesRenderFormat(ERPRenderFormat format) override;
...
protected:
  ...
  bool CopyDXVA2YUVBufferRP(ID3D11VideoDecoderOutputView* pView, YUVBufferRP *pBuf);
  ...
  // Disable DXVA rendererer
  // DXVA::CRPProcessorHD  *m_processor;
  std::vector<ERPRenderFormat> m_formats;
  ...
  CRPYUV2RGBShader*      m_colorShader;
  CRPConvolutionShader*  m_scalerShader;

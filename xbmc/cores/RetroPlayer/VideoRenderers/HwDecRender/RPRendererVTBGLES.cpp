...
CRPRendererVTB::CRPRendererVTB()
{
  ...
  {
    CLog::Log(LOGERROR, "CRPRendererVTB::CRPRendererVTB - Error creating texture cache (err: %d)", ret);
  }
...
void CRPRendererVTB::AddRPVideoPictureHW(RPVideoPicture &picture, int index)
{
...
void CRPRendererVTB::ReleaseBuffer(int idx)
{
...
int CRPRendererVTB::GetImageHook(YV12ImageRP *image, int source, bool readonly)
{
  return source;
}

CRPRenderInfo CRPRendererVTB::GetRenderInfo()
{
  CRPRenderInfo info;
...
bool CRPRendererVTB::LoadShadersHook()
{
  ...
  if (!m_textureCache)
  {
    CLog::Log(LOGNOTICE, "CRPRendererVTB::LoadShadersHook: no texture cache");
    return false;
  }
...
bool CRPRendererVTB::CreateTexture(int index)
{
...
void CRPRendererVTB::DeleteTexture(int index)
{
  CRenderBuffer &buf = m_vtbBuffers[index];
...
bool CRPRendererVTB::UploadTexture(int index)
{
  CRenderBuffer &buf = m_vtbBuffers[index];
  ...
  if (ret != kCVReturnSuccess)
  {
    CLog::Log(LOGERROR, "CRPRendererVTB::UploadTexture - Error uploading texture Y (err: %d)", ret);
    return false;
  }
  ...
  if (ret != kCVReturnSuccess)
  {
    CLog::Log(LOGERROR, "CRPRendererVTB::UploadTexture - Error uploading texture UV (err: %d)", ret);
    return false;
  }
...
void CRPRendererVTB::AfterRenderHook(int idx)
{
  CRenderBuffer &buf = m_vtbBuffers[idx];
  if (buf.m_fence && glIsSyncAPPLE(buf.m_fence))
  {
...
bool CRPRendererVTB::NeedBuffer(int idx)
{
  CRenderBuffer &buf = m_vtbBuffers[idx];
  if (buf.m_fence && glIsSyncAPPLE(buf.m_fence))
  {

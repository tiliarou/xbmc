...
CMMALBuffer *CMMALPool::GetBuffer(uint32_t timeout)
{
  ...
  if (mmal_buffer)
  {
    ...
    {
      MMAL::CMMALYUVBufferRP *yuv = new MMAL::CMMALYUVBufferRP(static_cast<MMAL::CDecoder *>(m_dec), shared_from_this(), m_mmal_format, m_width, m_height, aligned_width, aligned_height, m_size);
      if (yuv)
      {
...
CRPRenderInfo CMMALRenderer::GetRenderInfo()
{
...
void CMMALRenderer::AddRPVideoPictureHW(RPVideoPicture& pic, int index)
{
  if (m_format != RP_RENDER_FMT_MMAL)
  {
    assert(0);
    return;
  }
...
bool CMMALRenderer::Configure(unsigned int width, unsigned int height, unsigned int d_width, unsigned int d_height, float fps, unsigned flags, ERPRenderFormat format, void* hwPic, unsigned int orientation)
{
  CSingleLock lock(m_sharedSection);
  ReleaseBuffers();
  ...
  if (format != RP_RENDER_FMT_BYPASS && format != RP_RENDER_FMT_MMAL)
  {
...
int CMMALRenderer::GetImage(YV12ImageRP *image, int source, bool readonly)
{
  if (!image || source < 0 || m_format != RP_RENDER_FMT_MMAL)
  {
    if (g_advancedSettings.CanLogComponent(LOGVIDEO))
...
void CMMALRenderer::RenderUpdate(bool clear, DWORD flags, DWORD alpha)
{
  ...
  if (g_graphicsContext.GetStereoView() != RENDER_STEREO_VIEW_RIGHT)
  {
    ManageRenderArea();
    CRect view;
    CRPBaseRenderer::GetVideoRect(m_cachedSourceRect, m_cachedDestRect, view);
  }
...
{
  CSingleLock lock(m_sharedSection);
  if (!m_bConfigured || m_format != RP_RENDER_FMT_MMAL)
  {
    if (g_advancedSettings.CanLogComponent(LOGVIDEO))
...
void CMMALRenderer::PreInit()
{
  ...
  m_formats.clear();
  m_formats.push_back(RP_RENDER_FMT_MMAL);
  m_formats.push_back(RP_RENDER_FMT_BYPASS);

...
#include "RPRendererMediaCodec.h"
...
CRPRendererMediaCodec::CRPRendererMediaCodec()
{
#if defined(EGL_KHR_reusable_sync) && !defined(EGL_EGLEXT_PROTOTYPES)
  if (!eglCreateSyncKHR) {
...
void CRPRendererMediaCodec::AddRPVideoPictureHW(RPVideoPicture &picture, int index)
{
#ifdef DEBUG_VERBOSE
...
bool CRPRendererMediaCodec::RenderUpdateCheckForEmptyField()
{
  return false;
}

void CRPRendererMediaCodec::ReleaseBuffer(int idx)
{
  YUVBufferRP &buf = m_buffers[idx];
  if (buf.hwDec)
  {
...
bool CRPRendererMediaCodec::Supports(EINTERLACEMETHOD method)
{
...
EINTERLACEMETHOD CRPRendererMediaCodec::AutoInterlaceMethod()
{
  return VS_INTERLACEMETHOD_RENDER_BOB;
}

CRPRenderInfo CRPRendererMediaCodec::GetRenderInfo()
{
  CRPRenderInfo info;
...
bool CRPRendererMediaCodec::LoadShadersHook()
{
...
bool CRPRendererMediaCodec::RenderHook(int index)
{
  #ifdef DEBUG_VERBOSE
    unsigned int time = XbmcThreads::SystemClockMillis();
  #endif
...
int CRPRendererMediaCodec::GetImageHook(YV12ImageRP *image, int source, bool readonly)
{
  return source;
}

bool CRPRendererMediaCodec::CreateTexture(int index)
{
...
void CRPRendererMediaCodec::DeleteTexture(int index)
{
...
bool CRPRendererMediaCodec::UploadTexture(int index)
{
#ifdef DEBUG_VERBOSE
  unsigned int time = XbmcThreads::SystemClockMillis();
  int mindex = -1;

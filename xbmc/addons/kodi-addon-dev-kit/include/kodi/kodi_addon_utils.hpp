...
public:
   ...
   * @return The string translated to resolved path
   */
  static std::string TranslateSpecialProtocol(ADDON::CHelper_libXBMC_addon* frontend, const std::string& source)
  {
    std::string strReturn;
    if (frontend)
    {
      char* strPath = frontend->TranslateSpecialProtocol(source.c_str());
      if (strPath)
      {
        strReturn = strPath;
        frontend->FreeString(strPath);
      }
    }
    return strReturn;
  }
   ...
   */
  static std::string GetLocalizedString(ADDON::CHelper_libXBMC_addon* frontend, int dwCode)
  {
    std::string strReturn;
    if (frontend)
    {
